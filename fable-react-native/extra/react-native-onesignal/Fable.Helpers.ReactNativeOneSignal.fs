...
module OneSignal =
    ...
    let inline configure (props: IOneSignalOptions list) =
        ReactNativeOneSignal.Globals.OneSignal?configure(keyValueList CaseRules.LowerFirst props) |> ignore
    ...
    let cancelNotification (id:string) =
        ReactNativeOneSignal.Globals.OneSignal?cancelNotification(id) |> ignore


    /// Post Peer-to-Peer-Notfication.
    /// Allows you to send notifications from user to user or schedule ones in the future to be delivered to the current device.
    let postP2PNotification(message, data, userId: string) : unit =
        ReactNativeOneSignal.Globals.OneSignal?postNotification(message, data, userId) |> ignore